...
interface MerchantConnectCardProps {
  ...
  platformUrl: string;
  onConnectStart?: () => void;
  onConnectResult?: (result: ConnectResult) => void;
  submitLabel?: string;
}
  ...
  const handleConnect = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok && data.success) {
        result = {
          success: true,
          message: `Merchant store '${data.merchant?.name || merchantConfig.merchantName}' connected successfully. Credentials were saved with AES-256-GCM encryption.`,
        };
      } else {
        result = {
          success: false,
          message: data.error || 'Failed to connect merchant store.',
        };
      }
    } catch (err: any) {
      result = {
        success: false,
        message: `Could not connect to Platform API at ${platformUrl}: ${err.message}`,
      };
    }
        ...
        <div className="flex items-start gap-4">
          ...
          <div>
            ...
            <p className="mt-1 text-sm text-slate-400 leading-relaxed">
              ...
              {isConnected ? ' This store is already connected — submitting will update the saved profile.' : ''}
            </p>
          </div>
        </div>
      </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
            ...
            <div>
              ...
              <input
                ...
                placeholder="ShopExpress E-Commerce"
                required
              />
            </div>
